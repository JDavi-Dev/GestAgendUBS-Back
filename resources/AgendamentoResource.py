    ...
    def post(self):
        ...
        try:
            ...
            horario = db.session.execute(
                db.select(Horario).filter_by(id=agendamento_data['horario_id'])
            ).scalar_one_or_none()
            ...
            paciente = db.session.execute(
                db.select(Paciente).filter_by(id=agendamento_data['paciente_id'])
            ).scalar_one_or_none()
            ...
            conflito_paciente = db.session.execute(
                db.select(Agendamento).join(Horario).where(
                    and_(
                        Agendamento.paciente_id == agendamento_data['paciente_id'],
    ...
    def put(self, id):
        ...
        try:
            ...
            if 'status' in agendamento_data and agendamento_data['status'] == 'cancelado':
                ...
                data_hora_consulta = datetime.combine(horario.data, horario.hora_inicio)
                data_hora_consulta = data_hora_consulta.replace(tzinfo=timezone.utc)
                agora = datetime.now(timezone.utc)
                
                diferenca_horas = (data_hora_consulta - agora).total_seconds() / 3600

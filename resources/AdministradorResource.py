...
from helpers.auth import token_required, admin_required
...
class AdministradoresResouce(Resource):
    ...
    def post(self):
        ...
        senha = administrador_data.pop('senha', None)
        if not senha or len(senha) < 8:
            return {"mensagem": "A senha deve ter no mínimo 8 caracteres."}, 400

        if '@' not in str(administrador_data.get('email', '')):
        ...
        try:
            ...
            nova_credencial = Credencial(
                login=administrador_data['email'],
                senha=senha,
                tipo='administrador',
                referencia_id=novo_administrador.id
            )
...
class AdministradorResource(Resource):
    ...
    def put(self, id):
        ...
        if 'email' in administrador_data and '@' not in str(administrador_data['email']):
            return {"mensagem": "Informe um e-mail válido contendo '@'."}, 400
